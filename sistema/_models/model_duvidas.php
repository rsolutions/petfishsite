...
Class model_duvidas extends model{
	...
	public function ordem($grupo){ 
		$conexao = new mysql();
		$exec = $conexao->Executar("SELECT * FROM duvidas_ordem ORDER BY id desc limit 1");
		$data_ordem = $exec->fetch_object();
		if(isset($data_ordem->data)){
	...
	public function lista_grupos(){
		...
		$db = new mysql();
		$exec = $db->executar("SELECT * FROM duvidas_grupos order by titulo asc");		
	...
	public function carrega_grupo($codigo){
		$db = new mysql();
		$exec = $db->executar("SELECT * FROM duvidas_grupos where codigo='$codigo' ");
		return $exec->fetch_object();
	}
}