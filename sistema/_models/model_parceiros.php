...
Class model_parceiros extends model{
	...
	public function lista($grupo){
		...
		$conexao = new mysql();
		$exec = $conexao->Executar("SELECT * FROM parceiros_ordem WHERE grupo='$grupo' ORDER BY id desc limit 1");
	...
	public function lista_grupos(){
		...
		$db = new mysql();
		$exec = $db->executar("SELECT * FROM parceiros_grupos order by titulo asc");		
	...
	public function carrega_grupo($codigo){
		$db = new mysql();
		$exec = $db->executar("SELECT * FROM parceiros_grupos where codigo='$codigo' ");
		return $exec->fetch_object();
	}
}